...
# 1. Update CustomXAxisTick (remove any breakCenter slashes, just draw zero or normal ticks)
old_tick = """const CustomXAxisTick = ({ x, y, payload }: any) => {
  if (payload.value === 0 || isNaN(payload.value)) {"""
new_tick = """const CustomXAxisTick = ({ x, y, payload, zeroX }: any) => {
  const val = payload.value;
  if (val === zeroX || val === 0 || isNaN(val)) {"""
code = code.replace(old_tick, new_tick)

# 2. Update xTicks
old_xticks = """  const { xTicks, xDomain } = useMemo((): { xTicks: number[], xDomain: [number | 'auto', number | 'auto'] } => {
...
new_xticks = """  const { xTicks, leftDomain, rightDomain, zeroX, minX } = useMemo(() => {
...
code = code.replace(old_xticks, new_xticks)

# 3. Add split data properties after scatterData
split_data = """  const leftChartData = useMemo(() => chartData.filter(d => d.x <= zeroX * 2), [chartData, zeroX]);
...
"""
code = code.replace('  const updateRow =', split_data + '  const updateRow =')

# 4. Replace Chart JSX
import re
chart_frame_pattern = re.compile(r'<div className="chart-frame" ref=\{chartRef\} style=\{\{ position: \'relative\' \}\}>.*?</div>\s*</div>\s*</div>\s*\)\s*:\s*\(', re.DOTALL)

new_chart_frame = """<div className="chart-frame" ref={chartRef} style={{ position: 'relative', display: 'flex', flexDirection: 'column' }}>
...
code = chart_frame_pattern.sub(new_chart_frame, code)

# 5. Update Version
code = code.replace('v0.3.16', 'v0.4.9')
